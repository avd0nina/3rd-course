...
describe('Testing simplify function', ()=>
{
    ...
    test("a*(b+c)=>a*b+a*c", 4, parseSimplifyAndCost, 5, "a*(b+c)", toughIdentities);
    test("a*b+a*c=>a*(b+c)", 4, parseSimplifyAndCost, 5, "a*b+a*c", toughIdentities);
    test("aaa", 4, parseSimplifyAndCost, 2, "(2*a)+(2*a)", toughIdentities);
    test("a+(b+c) should not loop infinitely", 5, () => {
        const start = Date.now();
        const result = simplify(parseExpr("a+(b+c)"), toughIdentities());
        const duration = Date.now() - start;
        console.log(`a+(b+c) simplified in ${duration}ms => ${printExpr(result)}`);
        if (duration > 1000) {
            throw new Error(`Simplification took too long: ${duration}ms`);
        }
        return cost(result);
    }, 5);
});
