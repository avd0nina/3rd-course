...
Storage* initialize_storage(int capacity) {
    ...
    custom_mutex_init(&storage->head_mutex);
    custom_mutex_init(&global_mutex);
    return storage;
}

/**
 * Добавляет новый узел в конец списка.
 * Выделяет память под узел, копирует строку, инициализирует мьютекс узла и вставляет его в конец списка под защитой head_mutex.
...
void add_node(Storage *storage, const char *value) {
    ...
    new_node->next = NULL;
    custom_mutex_init(&new_node->sync);
 ...
 * Использует head_mutex для получения первого узла, затем передаёт эстафету мьютексам узлов. Безопасно при параллельных swap'ах.
 * @param storage Указатель на хранилище.
 ...
 * Проходит по списку, уничтожает мьютексы узлов, освобождает память узлов и хранилища.
 * @param storage Указатель на хранилище.
 */
void free_storage(Storage *storage) {
    ...
    while (current != NULL) {
        ...
        custom_mutex_destroy(&current->sync);
        free(current);
        current = next;
    }
    custom_mutex_destroy(&storage->head_mutex);
    custom_mutex_destroy(&global_mutex);
    free(storage);
}
