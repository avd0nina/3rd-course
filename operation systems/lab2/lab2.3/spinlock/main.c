...
void *swap_thread(void *data) {
    ...
    while (running) {
        ...
        for (int i = 0; i < pos && first && first->next; i++) {
            second = first->next;
            pthread_spin_lock(&second->sync);
            if (i == pos - 1 && second->next != NULL) {
                third = second->next;
                pthread_spin_lock(&third->sync);
                found = 1;
                break;
            }
        ...
        if (!found) {
            pthread_spin_unlock(&first->sync);
            if (second) pthread_spin_unlock(&second->sync);
            usleep(1000);
            continue;
        }
        ...
        // Освобождаем спинлоки в обратном порядке
...
int main(int argc, char *argv[]) {
    ...
    printf("SPINLOCK VERSION - Capacity: %d\n", capacity);
