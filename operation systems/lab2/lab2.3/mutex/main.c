...
void *swap_thread(void *data) {
    ...
    while (running) {
        ...
        if (!found) {
            pthread_mutex_unlock(&first->sync);
            if (second) pthread_mutex_unlock(&second->sync);
            usleep(1000);
            continue;
        }
        ...
        // Освобождаем мьютексы в обратном порядке
...
int main(int argc, char *argv[]) {
    ...
    printf("MUTEX VERSION - Capacity: %d\n", capacity);
