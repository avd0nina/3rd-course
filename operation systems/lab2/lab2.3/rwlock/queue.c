 ...
 * Выделяет память под узел, копирует строку, инициализирует rwlock узла и вставляет его в конец списка под защитой head_mutex.
 ...
 * Использует head_mutex для получения первого узла, затем передаёт эстафету rwlock'ам узлов (read locks). Безопасно при параллельных swap'ах.
 * @param storage Указатель на хранилище.
 ...
 * Проходит по списку, уничтожает rwlock'и узлов, освобождает память узлов и хранилища.
 * @param storage Указатель на хранилище.
 */
