...
static void handle_client(void *arg) {
    ...
    char *request = NULL;
    ssize_t request_len = receive_full_data(ctx->client_socket, &request);
...
static int get_host_port(const char *host_port, char *host, int *port) {
    ...
    if (ret == 0) {
        ...
        proxy_log("Host and port getting error: no host or/and port");
        return ERROR;
    } else {
...
static int parse_response(const char *response, size_t response_len, int *status, size_t *content_len, size_t *content_length_header) {
    ...
    char *end = NULL;
    long temp = strtol(content_length_value, &end, 0);
    ...
    *content_length_header = (size_t)temp;
